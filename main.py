...
# --- 3. لوحة التحكم والمساعد الذكي ---
    ...
    with rep_tab1:
        ...
        # --- المساعد الذكي المتطور (النسخة المرنة الجديدة) ---
        st.subheader("🤖 المساعد الذكي المتطور (تنفيد الأوامر عبر المحادثة)")
        st.caption("اسأل عن أرباحك ومبيعاتك، أو أمره بإضافة منتج مباشرة (مثلاً: ضيف شاحن بسعر 5000 وعدد 10)")
        ...
        if user_ai_query:
            ...
            if "ضيف" in q_lower or "أضف" in q_lower:
                    ...
                    if price_match and qty_match:
                        ...
                        if name_match:
                            ...
                            if p_name:
                                ...
                                st.warning("🤖 **المساعد الذكي:** يرجى كتابة اسم المنتج بوضوح بعد كلمة ضيف.")
                        else:
                            st.warning("🤖 **المساعد الذكي:** لم أستطع تمييز اسم المنتج. اكتب هكذا: (ضيف [اسم المنتج] بسعر [السعر] وعدد [العدد]).")
                    else:
                        st.warning("🤖 **المساعد الذكي:** يرجى التأكد من ذكر السعر والعدد بوضوح في الأمر.")
                ...
                st.warning("🤖 **المساعد الذكي:** لم أفهم أمرك بدقة. يمكنك سؤالي عن (المبيعات)، (المنتجات)، (الديون)، أو كتابة أمر إضافة مثل: (ضيف [اسم المنتج] بسعر [السعر] وعدد [العدد]).")
