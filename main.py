...
with tabs[3]: # الفواتير (الجزء الأقوى)
    st.header("🧾 الفواتير")
    if st.button("🧹 تصفير وتنظيف الكل"):
    ...
    for _, row in invoices.iterrows():
        with st.expander(f"فاتورة #{row['rowid']} | {row['customer_name']}"):
            try:
                ...
                # إذا فشلت القراءة، لا ينهار البرنامج، يعرض النص الخام (Raw)
                st.warning("⚠️ هذه الفاتورة بصيغة قديمة أو تالفة، إليك محتواها الأصلي:")
                st.code(row['items']) 
                if st.button(f"🗑️ حذف هذه الفاتورة #{row['rowid']}", key=f"d_{row['rowid']}"):
