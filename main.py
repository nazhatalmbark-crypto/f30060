...
# --- دالة إنشاء PDF (مُعدلة لتكون صارمة مع الأرقام) ---
def generate_pdf(invoice_id, customer_name, items, total):
    ...
    # تحويل إجباري لكل القيم إلى int لتجنب أي خطأ
    for item, data in items.items():
        p = int(data['price'])
        q = int(data['qty'])
        pdf.cell(200, 10, txt=f"{item} | الكمية: {q} | السعر: {p}", ln=True)
...
with tabs[0]: # البيع
    ...
    if prods.empty:
        st.warning("⚠️ المخزن فارغ! يرجى إضافة مواد.")
    else:
        ...
        if st.button("➕ أضف للسلة"):
            if selected_customer == "اختر عميل...": st.error("اختر عميلاً!")
                ...
                st.session_state.cart[item_select] = {'price': int(price), 'qty': int(qty_input)}
                st.rerun()
...
with tabs[3]: # الفواتير (نظام حماية كامل)
    st.header("🧾 الفواتير")
    ...
    for _, row in invoices.iterrows():
        with st.expander(f"فاتورة #{row['rowid']} | {row['customer_name']}"):
            try:
                items_raw = ast.literal_eval(row['items'])
                # إجبار البيانات على أن تكون أرقام صحيحة نظيفة
                items = {k: {'price': int(v['price']), 'qty': int(v['qty'])} for k, v in items_raw.items()}
                ...
                if st.button(f"🗑️ حذف الفاتورة #{row['rowid']}", key=f"d_{row['rowid']}"):
...
with tabs[5]: # كشف حساب
    st.header("📊 كشف حساب")
    sel = st.selectbox("اختر", ["اختر..."] + pd.read_sql("SELECT name FROM customers", conn)['name'].tolist())
    if sel != "اختر...":
        st.table(pd.read_sql("SELECT * FROM invoices WHERE customer_name = ?", conn, params=(sel,)))
