...
# --- الشريط الجانبي ---
st.sidebar.title("🛠️ التحكم والأمان")
if not st.session_state.is_unlocked:
    st.sidebar.warning("⚠️ النسخة التجريبية (5 مواد فقط)")
...
except: st.sidebar.error("لا يوجد قاعدة بيانات")
...
with tabs[1]: # الأرشيف
    ...
    for i in invs:
        with st.container(border=True):
            ...
            st.download_button(f"📥 PDF فاتورة {i[0]}", pdf_data, f"inv_{i[0]}.pdf")

with tabs[2]: # المخزن (مع الشبكة والقيود)
    ...
    with st.expander("➕ إضافة مادة جديدة", expanded=True):
        if not st.session_state.is_unlocked and count >= 5:
            st.warning("⚠️ النسخة التجريبية محدودة بـ 5 مواد فقط. فعل النسخة الكاملة!")
        else:
