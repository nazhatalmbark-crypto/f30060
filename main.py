...
if not st.session_state.is_vip:
    ...
    if st.sidebar.button("تفعيل النسخة المدفوعة"):
        if vip_code.strip() == "YASSER2026":
            ...
            st.sidebar.error("كود التفعيل غير صحيح! (الكود هو: YASSER2026)")
else:
...
with tab8:
    ...
    1. **➕ إضافة مادة جديدة (Tab 1):**
       ...
       * **ملاحظة:** في النسخة المجانية، يمكنك إضافة حتى 5 منتجات فقط. لتفعيل الحساب وإزالة القيود بشكل كامل، استخدم كود التفعيل VIP الموجود أسفل القائمة الجانبية: `YASSER2026`.
