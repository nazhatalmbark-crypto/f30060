...
# --- 3. جرد المخزن ---
    ...
    for _, row in products.iterrows():
        ...
        if c2.button(f"حذف المادة", key=f"del_db_{row['rowid']}"):
...
# --- 4. أرشيف الفواتير ---
    ...
    # عرض فاتورة محددة إذا تم اختيارها
    if 'view_invoice' in st.session_state and st.session_state.view_invoice:
        ...
        st.write(f"**التاريخ:** {inv['timestamp']}")
        st.markdown("---")
        st.write(f"{inv['items']}") # عرض المواد
        ...
        for _, row in invoices.iterrows():
            ...
            if c2.button("عرض الفاتورة", key=f"view_{row['rowid']}"):
