...
with tab10:
    st.subheader("📖 دليل الاستخدام الشامل لنظام Yasser Web لإدارة المبيعات والمخازن")
    st.write("مرحباً بك في الدليل التعريفي الشامل الخاص بالنظام. يوضح هذا الدليل كافة الأقسام وكيفية التعامل مع المنظومة بشكل تفصيلي:")
    ...
    * **تنبيهات الذكاء البرمجي:** النظام ينبهك فوراً إذا كان سعر البيع أقل من سعر الشراء لتجنب البيع بخسارة، كما يراعي حدود النسخة المجانية والمدفوعة (VIP).
    """)

    st.markdown("---")
    st.markdown("### 2️⃣ تبويب جرد المخزن والباركود (Inventory & Barcode)")
    st.write("""
    * **الغرض:** عرض كافة المواد المتوفرة في المخزن مع مراقبة مستويات النفاذ وتوليد الباركود.
    * **المميزات:** 
      * بحث سريع وسهل حسب اسم المادة أو الباركود.
      * تنبيهات تلقائية (Low Stock Alerts) عند وصول الكمية إلى قطعتين أو أقل.
      * توليد باركود بصري لكل مادة وقابليته للطباعة.
      * إمكانية إضافة المواد مباشرة إلى سلة المبيعات بضغطة زر واحدة.
    """)
    ...
    * **طريقة الاستخدام:** أدخل اسم العميل، رقم الهاتف، واختيار المحافظة العراقية من القائمة، مع حفظ العنوان والملاحظات لسهولة الرجوع إليها عند إصدار الفواتير الآجلة.
    """)
    ...
    * **المميزات:** تسجيل أسماء الموردين، أرقام هواتفهم، ونوع التخصص التجاري لضمان التواصل السريع والمستمر لتوفير النقص في المخزن.
    """)

    st.markdown("---")
    st.markdown("### 5️⃣ تبويب سلة المبيعات وإتمام الفاتورة (Checkout)")
    st.write("""
    * **الغرض:** النافذة المركزية لإنهاء عمليات البيع للزبائن.
    * **الخطوات:**
      1. مراجعة المواد المختارة في السلة وتعديل الكميات المطلوبة.
      2. اختيار اسم الزبون المسجل مسبقاً في النظام.
      3. تحديد طريقة الدفع (نقد بالكامل، آجل بالكامل، أو دفعة جزئية).
      4. الضغط على زر إتمام البيع ليقوم النظام تلقائياً بخصم المواد من المخزن وتوليد رقم الفاتورة وحفظها في السجل والذمم.
    """)

    st.markdown("---")
    st.markdown("### 6️⃣ تبويب سجل الفواتير وواتساب (Invoice History)")
    st.write("""
    * **الغرض:** مراجعة جميع الفواتير السابقة وإدارتها بالكامل.
    * **المميزات:**
      * استعراض حالة الدفع (مدفوع بالكامل أو يوجد دين متبقي).
      * توليد **QR Code** تفصيلي خاص بكل فاتورة لمسحه سريعاً.
      * زر مخصص لتحميل الفاتورة بتنسيق **PDF** مرتب وجاهز للطباعة.
      * زر إرسال الفاتورة والمطالبة المالية مباشرة عبر **واتساب** للزبون بضغطة زر.
    """)

    st.markdown("---")
    st.markdown("### 7️⃣ تبويب صندوق الوردية والمصاريف (Shift Box)")
    st.write("""
    * **الغرض:** السيطرة المالية على السيولة النقدية اليومية وسحب النثريات وحساب الأرباح بدقة.
    * **المميزات:** تسجيل المصاريف اليومية (مثل الإيجار والكهرباء وسحوبات الإدارة)، مع احتساب صافي النقد الفعلي بالصندوق وصافي الربح الحقيقي بعد خصم تكلفة البضائع والمصاريف.
    """)

    st.markdown("---")
    st.markdown("### 8️⃣ تبويب الرسوم البيانية والتقارير (Charts & Reports)")
    st.write("""
    * **الغرض:** تقديم نظرة إدارية عليا عبر مؤشرات أداء مرئية (KPIs) ورسوم بيانية تفصيلية تعكس حركة المبيعات، الإيرادات، الديون المعلقة، والأرباح الصافية بأسلوب بصري احترافي.
    """)

    st.markdown("---")
    st.markdown("### 9️⃣ تبويب سجل النشاطات (Audit Trail / Logs)")
    st.write("""
    * **الغرض:** الرقابة الأمنية والتدقيق الإداري على كافة العمليات التي تتم داخل النظام (تسجيل الدخول، إضافة المنتجات، إتمام المبيعات، مسح السجلات) مع توثيق الوقت واسم المستخدم وصلاحيته بدقة مطلقة.
    """)

    st.markdown("---")
    st.markdown("### 🔟 إعدادات الحساب والنسخ الاحتياطي (Sidebar)")
    st.write("""
    * **الغرض:** حماية بياناتك الشخصية والتجارية من الضياع.
    * **المميزات:**
      * التبديل الفوري بين اللغة العربية والإنجليزية.
      * **تحميل نسخة احتياطية:** تصدير كافة بيانات المحل (العملاء، الموردين، الفواتير، المصاريف، السجلات) إلى ملف بصيغة `JSON` وحفظه على جهازك.
      * **استعادة البيانات:** رفع ملف النسخة الاحتياطية القديم في أي وقت لاسترجاع البيانات بالكامل.
      * **تفعيل النسخة المدفوعة (VIP):** إدخال كود التفعيل لإزالة القيود المفروضة على النسخة المجانية.
    """)
