...
# --- إدارة الجلسة (تسجيل الدخول والنسخة) ---
...
if not st.session_state.logged_in:
    ...
    if not st.session_state.is_unlocked:
        st.sidebar.warning("⚠️ النسخة المجانية (5 مواد فقط)")
            ...
            for idx, itm in enumerate(items):
                with cols[idx % 3]:
                    with st.container(border=True):
                        ...
                        if st.button("➕ إضافة", key=f"pos_add_{itm[0]}"):
                            found = False
        ...
        if invs:
            for i in invs:
                with st.container(border=True):
                    ...
                    st.download_button(f"📥 تحميل PDF #{i[0]}", pdf_data, f"inv_{i[0]}.pdf", key=f"pdf_{i[0]}")
        else:
    ...
    with tabs[2]: # المخزن
        ...
        with st.expander("➕ إضافة مادة جديدة"):
            if not st.session_state.is_unlocked and count >= 5:
                st.warning("⚠️ وصل الحد الأقصى للنسخة المجانية (5 مواد). أدخل رمز (2009) في الشريط الجانبي للتفعيل!")
            else:
        ...
        with st.expander("🔄 تزويد كمية مادة موجودة"):
            ...
            if existing:
                ...
                if st.button("إضافة للمخزن"):
                    ...
                    st.success(f"تمت إضافة {add_qty} قطعة للمادة: {sel_item}")
                    st.rerun()
            else:
    ...
    with tabs[4]: # العملاء (الحقول الأربعة المستقلة تماماً)
        ...
        if custs:
            for c in custs:
                with st.container(border=True):
                    ...
                    with col2:
                        if st.button("حذف", key=f"del_c_{c[0]}"):
