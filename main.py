...
# --- تعريف التبويبات الرئيسية (في البداية وقبل أي استدعاء) ---
...
with tabs[2]: # المخزن (عرض شبكي 3 أعمدة + ربح القطعة + تزويد الكمية + قيد الـ 5 مواد)
    st.subheader("📦 إدارة المخزن (عرض شبكي)")
    count = run_query("SELECT COUNT(*) FROM inventory", fetch=True)[0][0]
    
    with st.expander("➕ إضافة مادة جديدة للمخزن", expanded=False):
    ...
    with st.expander("🔄 إضافة كمية لمادة موجودة (تزويد المخزن)", expanded=False):
...
with tabs[3]: # الأرباح (جدول تفصيلي لكل منتج والكميات المباعة)
    ...
    if items:
        for itm in items:
            ...
            profit_data.append({
                ...
                "ربح القطعة": profit_per_unit,
                "إجمالي الربح": profit_per_unit * sold_qty
            })
        ...
        if profit_data:
            ...
            st.info("لا توجد بيانات أرباح بعد.")
    else:
        st.info("لا توجد مواد مسجلة في المخزن لحساب الأرباح.")
