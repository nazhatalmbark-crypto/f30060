...
# مكتبات تعديل وعكس الحروف العربية
...
# دالة لتسجيل الخط العربي وتنزيله مؤقتاً للـ PDF
@st.cache_resource
...
# شاشة تسجيل الدخول وإنشاء الحساب (التبويبات الأساسية)
if not st.session_state.logged_in_user:
    st.subheader("🔐 بوابة الدخول وإنشاء الحساب للمحل")
...
# التطبيق الكامل بعد تسجيل الدخول
...
# التبويبات الستة الكاملة والمطلوبة
...
with tab1:
    ...
    if not st.session_state.is_vip and current_count >= 5:
        st.warning("⚠️ **تنبيه النسخة المجانية:** وصلت للحد الأقصى (5 منتجات). فعّل النسخة المدفوعة بكود (`YASSER2026`) من القائمة الجانبية لإضافة منتجات بلا حدود!")
    else:
...
with tab2:
    st.subheader("🧱 عرض بضائع المحل والمخزن")
    try:
...
with tab5:
    st.subheader("📄 سجل الفواتير وتحميلها PDF")
...
with tab6:
    st.subheader("📊 تقارير الأرباح والجرد المالي")
    if not st.session_state.is_vip:
        st.info("💡 النسخة المجانية (تجريبية). فعّل النسخة المدفوعة بكود `YASSER2026` من القائمة الجانبية لعرض بياناتك وحساباتك الحقيقية.")
    else:
        try:
            ...
            if res_stats and res_stats.data:
                ...
                m1.metric("إجمالي عدد القطع", f"{sum(p['quantity'] for p in res_stats.data)}")
